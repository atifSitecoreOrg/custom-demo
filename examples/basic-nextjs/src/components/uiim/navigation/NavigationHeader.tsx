...
/* JetourUAE variant — dark sticky nav matching jetouruae.com */
export const JetourUAE = ({ fields, params, page }: NavigationHeaderProps): JSX.Element => {
  const { styles, RenderingIdentifier } = params;
  const isEditing = page?.mode?.isEditing;
  const [menuOpen, setMenuOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  ...
  return (
    <div className={cn('component navigation-header', styles)} id={RenderingIdentifier}>
      <header
        className={cn('fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300', scrolled ? 'shadow-lg' : '')}
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 h-14 sm:px-8">
          ...
          <Link href="/" className="flex items-center shrink-0 order-first rtl:order-last">
            {brandLogo?.value?.src ? (
              ...
              <span className="text-base font-bold tracking-[0.15em] uppercase text-white font-[var(--brand-heading-font,inherit)]">
                JETOUR
              </span>
            )}
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-7 rtl:flex-row-reverse">
            {links.map((item) => (
              <ContentSdkLink
                key={item.id}
                field={item.linkUrl?.jsonValue}
                className="text-[12px] font-medium uppercase tracking-wider text-white/70 transition-colors hover:text-white"
              >
                {item.linkText?.jsonValue?.value && (
                  <Text field={item.linkText?.jsonValue} />
                )}
          ...
          <div className="flex items-center gap-3 order-last rtl:order-first rtl:flex-row-reverse">
            {(datasource.ctaLink?.jsonValue?.value?.href || isEditing) && (
              <ContentSdkLink
                ...
                className="hidden md:inline-flex items-center px-5 py-2 text-[11px] font-bold uppercase tracking-widest transition-opacity hover:opacity-90"
                style={{
        ...
        {/* Mobile menu */}
        {menuOpen && (
          <div className="md:hidden border-t border-white/10" style={{ backgroundColor: '#111111' }}>
            <div className="px-6 py-4 flex flex-col gap-4">
              ...
              {(datasource.ctaLink?.jsonValue?.value?.href || isEditing) && (
                <ContentSdkLink
                  field={datasource.ctaLink?.jsonValue}
                  className="mt-2 inline-flex items-center justify-center px-5 py-2.5 text-sm font-bold uppercase tracking-widest"
                  style={{ backgroundColor: 'var(--brand-accent, #c8102e)', color: '#ffffff', borderRadius: '2px' }}
                >
                  {datasource.ctaLabel?.jsonValue?.value && (
                    <Text field={datasource.ctaLabel?.jsonValue} />
                  )}
